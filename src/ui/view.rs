use crate::app::{App, BiblePane, View};
...
pub fn ui(f: &mut Frame, app: &mut App) {
    ...
    if let Some(view) = app.view_stack.last() {
        ...
        match view {
            ...
            View::ConfessionView => { let r = app.cfw.iter().map(|c| Row::new(vec![c.chapter.to_string(), c.title.clone()])).collect::<Vec<_>>(); render_table(f,theme,area,"Confissão",&["Cap.","Título"],&r,&mut app.cfw_list_state); }
            ...
            View::ActionEdit { text, .. } => render_input(f, theme, area, "Ação de Santificação", text),
            View::ResolutionEdit { text, .. } => render_input(f, theme, area, "Resolução", text),
            View::DiarioEdit { text, .. } => render_input(f, theme, area, "Entrada do Diário", text),
        }
    }
...
fn render_cfw_detail(f: &mut Frame, app: &App, style: Style, area: Rect) { let t = if let Some(s) = app.cfw_list_state.selected() {format!("{}\n\n{}", app.cfw[s].title, app.cfw[s].text)} else {String::new()}; f.render_widget(Paragraph::new(t).wrap(Wrap{trim:true}).block(Block::default().borders(Borders::ALL).title("Confissão").style(style)), area); }
...
fn render_bible_view(f: &mut Frame, app: &mut App, style: Style, area: Rect) {
    let chunks = Layout::new(Direction::Horizontal, [Constraint::Percentage(30),Constraint::Percentage(15),Constraint::Percentage(55)]).split(area);
    let active = Style::new().add_modifier(Modifier::REVERSED);
    let books: Vec<ListItem> = app.books.iter().map(|b| ListItem::new(b.name.clone())).collect();
    let mut book_b = Block::default().borders(Borders::ALL).title("Livros").style(style);
    if app.bible_state.active_pane == BiblePane::Books { book_b = book_b.border_style(active); }
    f.render_stateful_widget(List::new(books).block(book_b).highlight_style(active), chunks[0], &mut app.bible_state.book_list_state);
    let chaps: Vec<ListItem> = app.bible_state.chapters.iter().map(|c| ListItem::new(c.to_string())).collect();
    let mut chap_b = Block::default().borders(Borders::ALL).title("Cap.").style(style);
    if app.bible_state.active_pane == BiblePane::Chapters { chap_b = chap_b.border_style(active); }
    f.render_stateful_widget(List::new(chaps).block(chap_b).highlight_style(active), chunks[1], &mut app.bible_state.chapter_list_state);
    let verses: Vec<Line> = app.bible_state.verses.iter().map(|v| Line::from(format!("{} {}", v.verse, v.text))).collect();
    let mut text_b = Block::default().borders(Borders::ALL).title("Texto").style(style);
    if app.bible_state.active_pane == BiblePane::Text { text_b = text_b.border_style(active); }
    f.render_widget(Paragraph::new(Text::from(verses)).block(text_b).wrap(Wrap { trim: true }).scroll((app.bible_state.text_scroll, 0)), chunks[2]);
}
fn render_footer(f: &mut Frame, app: &App, style: Style, area: Rect) {
    let text = match app.view_stack.last() {
        Some(View::PsalmList) => "[j/k] Mover | [Enter] Letra | [s] Parar | [t/c] Tocar | [1/2/3] Módulos",
        Some(View::PietyMenu) | Some(View::StudyMenu) | Some(View::FaithSymbolsMenu) => "[1/2/3] Selecionar | [Esc] Voltar",
        Some(View::BibleView) => "[j/k] Scroll/Mover | [l/Tab] Painel | [Enter] Sel. | [Esc] Voltar",
        Some(View::ActionList) | Some(View::ResolutionList) | Some(View::DiarioList) => "[n] Novo | [d] Del | [Enter] Editar | [j/k] Mover | [Esc] Voltar",
        Some(View::ActionEdit {..}) | Some(View::ResolutionEdit {..}) | Some(View::DiarioEdit {..}) => "[Enter] Salvar | [Esc] Cancelar",
        _ => "[j/k] Mover | [Enter] Detalhes | [Esc] Voltar | [q] Sair"
    };
    f.render_widget(Paragraph::new(text).block(Block::default().borders(Borders::ALL).title("Ajuda").style(style)), area);
}
