...
#[derive(Default, Copy, Clone, PartialEq, Eq)] pub enum CfwPane { #[default] Chapters, Sections }
...
#[derive(Default)]
pub struct CfwState {
    pub active_pane: CfwPane,
    pub chapter_list_state: ListState,
    pub section_list_state: ListState,
    pub chapters: Vec<i32>,
    pub sections: Vec<Cfw>, // Apenas as seções do capítulo selecionado
}
...
pub struct App {
    ...
    pub cmw_list_state: TableState, pub bcw_list_state: TableState,
    pub bible_state: BibleState, pub cfw_state: CfwState,
}
impl App {
    pub fn new() -> Result<Self, AppError> {
        let canon_repo = CanonRepo::new()?;
        let piety_repo = PietyRepo::new()?;
        let cfw_data = canon_repo.get_all_cfw()?;
        let unique_chapters: HashSet<i32> = cfw_data.iter().map(|c| c.chapter).collect();
        let mut chapters_sorted: Vec<i32> = unique_chapters.into_iter().collect();
        chapters_sorted.sort();
        ...
        Ok(Self {
            ...
            cfw: cfw_data, cmw: canon_repo.get_all_cmw()?, bcw: canon_repo.get_all_bcw()?, books: canon_repo.get_all_books()?,
            ...
            cmw_list_state: TableState::default(), bcw_list_state: TableState::default(),
            bible_state: BibleState::default(),
            cfw_state: CfwState { chapters: chapters_sorted, ..Default::default() },
        })
    }
    ...
    // Métodos da CFW
    pub fn select_cfw_chapter(&mut self) {
        if let Some(sel) = self.cfw_state.chapter_list_state.selected() {
            let chap_num = self.cfw_state.chapters[sel];
            self.cfw_state.sections = self.cfw.iter().filter(|c| c.chapter == chap_num).cloned().collect();
            self.cfw_state.section_list_state.select(Some(0));
            self.cfw_state.active_pane = CfwPane::Sections;
        }
    }
    pub fn next_cfw_pane(&mut self) { self.cfw_state.active_pane = match self.cfw_state.active_pane { CfwPane::Chapters => CfwPane::Sections, CfwPane::Sections => CfwPane::Chapters }; }
